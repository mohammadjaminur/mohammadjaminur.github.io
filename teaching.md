---
layout: default
title: Teaching
description: Mohammad Jaminur Islam's teaching appointments, discussion sections, lab instruction, and guest lectures at UC Riverside.
---

<div class="wrap wrap-wide">
  <p class="kicker">Teaching</p>
  <h1>Teaching experience</h1>
  <p class="lede">I have served as a Graduate Teaching Assistant for six courses at UC Riverside since Fall 2022. I have led discussion sections and labs and given guest lectures on deep neural networks and generative models.</p>

  <h2>Course highlights</h2>

  <div class="course-block">
    <h3><span class="course-code">CS 170</span> — Introduction to Artificial Intelligence</h3>
    <p class="meta">Discussion sections · 7 terms, Fall 2022 – Summer 2026</p>
    <p>Across seven terms, I led discussions spanning search algorithms and optimization (including A* and simulated annealing), adversarial games, constraint satisfaction, Markov decision processes, reinforcement learning, and probabilistic reasoning. The Summer 2026 course focused entirely on reinforcement learning, and I also gave a guest lecture on deep neural networks during that term.</p>
  </div>

  <div class="course-block">
    <h3><span class="course-code">CS 105</span> — Data Analysis Methods</h3>
    <p class="meta">Lab instruction · Summer 2026 &amp; Spring 2026</p>
    <p>I led weekly labs on statistics, exploratory data analysis, k-nearest neighbors, clustering, and unsupervised deep learning. I also gave one guest lecture on deep neural networks and another on generative models, including GANs and autoencoders.</p>
  </div>

  <h2>Appointments</h2>
  <ul class="term-list">
    <li>
      <p class="term">Summer 2026</p>
      <ul>
        <li><span class="course-code">CS 170</span> — Introduction to Artificial Intelligence <em>(guest lecture: deep neural networks)</em></li>
        <li><span class="course-code">CS 105</span> — Data Analysis Methods <em>(guest lectures: deep neural networks; GANs &amp; autoencoders)</em></li>
      </ul>
    </li>
    <li>
      <p class="term">Spring 2026</p>
      <ul>
        <li><span class="course-code">CS 105</span> — Data Analysis Methods</li>
      </ul>
    </li>
    <li>
      <p class="term">Winter 2026</p>
      <ul>
        <li><span class="course-code">CS 252</span> — Data Analytics and Exploration</li>
        <li><span class="course-code">CS 150</span> — Automata and Formal Languages</li>
      </ul>
    </li>
    <li>
      <p class="term">Fall 2025</p>
      <ul>
        <li><span class="course-code">CS 170</span> (023) — Introduction to Artificial Intelligence</li>
        <li><span class="course-code">CS 175</span> (021) — Entrepreneurship in Computing</li>
      </ul>
    </li>
    <li>
      <p class="term">Spring 2025</p>
      <ul>
        <li><span class="course-code">CS 170</span> (023, 024) — Introduction to Artificial Intelligence</li>
      </ul>
    </li>
    <li>
      <p class="term">Winter 2025</p>
      <ul>
        <li><span class="course-code">CS 170</span> (021, 022) — Introduction to Artificial Intelligence</li>
      </ul>
    </li>
    <li>
      <p class="term">Fall 2024</p>
      <ul>
        <li><span class="course-code">CS 160</span> (023) — Concurrent Programming and Parallel Systems</li>
      </ul>
    </li>
    <li>
      <p class="term">Spring 2023</p>
      <ul>
        <li><span class="course-code">CS 170</span> (021, 022) — Introduction to Artificial Intelligence</li>
      </ul>
    </li>
    <li>
      <p class="term">Winter 2023</p>
      <ul>
        <li><span class="course-code">CS 170</span> (021, 022) — Introduction to Artificial Intelligence</li>
      </ul>
    </li>
    <li>
      <p class="term">Fall 2022</p>
      <ul>
        <li><span class="course-code">CS 170</span> — Introduction to Artificial Intelligence</li>
      </ul>
    </li>
  </ul>
</div>
